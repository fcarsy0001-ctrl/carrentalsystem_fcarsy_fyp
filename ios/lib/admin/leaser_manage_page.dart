...
class _LeaserManagePageState extends State<LeaserManagePage> {
  ...
  Future<void> _refresh() async {
    // IMPORTANT: setState callback must return void (not a Future).
    setState(() {
      _future = _load();
    });
    await _future;
  ...
  Future<String> _resolveAuthUid(Map<String, dynamic> row) async {
    // Prefer leaser.auth_uid, but fall back to app_user.auth_uid via user_id.
    final direct = _s(row['auth_uid']).trim();
  ...
  Future<void> _toggleActive(Map<String, dynamic> row) async {
    ...
    // After approval, leaser should be considered "Active" in day-to-day operations.
    final next = (st == 'inactive' || st.contains('deactive')) ? 'Active' : 'Inactive';
  ...
  Future<void> _delete(Map<String, dynamic> row) async {
    ...
    final id = _s(row['leaser_id']).trim();
    var authUid = _s(row['auth_uid']).trim();
    if (id.isEmpty) return;
    ...
    try {
      ...
      if (authUid.isEmpty) authUid = await _resolveAuthUid(row);
      if (authUid.isEmpty) throw Exception('Missing auth_uid for this leaser. Cannot delete Auth user safely.');
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text('Leaser ID: $id', style: TextStyle(color: Colors.grey.shade700)),
                                  if (email.isNotEmpty) ...[
                                    const SizedBox(height: 2),
                                    Text('Email: $email', style: TextStyle(color: Colors.grey.shade700)),
                                  ],
                                  const SizedBox(height: 2),
                                  Text('Status: $status', style: TextStyle(color: Colors.grey.shade700)),
                                ],
                              ),
                            ),
                            Column(
                              children: [
                                ...
                                IconButton(
                                  ...
                                  icon: Icon(
                                    (status.trim().toLowerCase() == 'inactive') ? Icons.power : Icons.power_off,
                                  ),
                                  tooltip: (status.trim().toLowerCase() == 'inactive') ? 'Activate' : 'Deactivate',
                                ),
                                IconButton(
                                  onPressed: () => _delete(r),
