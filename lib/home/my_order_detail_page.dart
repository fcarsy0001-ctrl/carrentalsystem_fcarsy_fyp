...
class _MyOrderDetailsPageState extends State<MyOrderDetailsPage> {
  ...
  Future<void> _insertBookingExtraChargeFlexible({
    required Map<String, dynamic> basePayload,
  }) async {
    const chargeTypeCandidates = <String>[
      'overtime',
      'late_return',
      'late return',
      'Late return',
    ...
    for (final candidate in chargeTypeCandidates) {
    ...
    throw Exception('Failed to create overtime billing.');
  }

  Future<void> _createAutoOvertimeBill({
    ...
    final cleanHours = math.max(1, overtimeHours);
    final note = 'Auto-generated by system after order drop-off. Late return penalty for $cleanHours overtime hour(s).';

    await _insertBookingExtraChargeFlexible(
      basePayload: {
        'booking_id': bookingId,
        'user_id': userId,
        'title': 'Overtime penalty bill',
        'description': note,
        'remark': note,
        'notes': note,
        'amount': roundedAmount,
        'charge_status': 'pending',
        'created_at': issuedAt.toUtc().toIso8601String(),
      },
    );
  }
  ...
  Future<void> _dropoffCar() async {
    ...
    final overtimeHours = _overtimeHoursRoundedUp(effectiveDropoffTime: dropoffTime);
    try {
      Map<String, _UploadedEvidenceAsset> uploadedEvidence = const <String, _UploadedEvidenceAsset>{};
      var evidenceStoredRemotely = false;
      Object? evidenceError;
      ...
      final bookingPatch = <String, dynamic>{
      ...
      try {
        await _supa.from('booking').update(bookingPatch).eq('booking_id', bookingId);
      } catch (_) {
        await _supa.from('booking').update({
          'booking_status': 'Inactive',
          'dropoff_completed_at': dropoffTime.toUtc().toIso8601String(),
      ...
      if (penalty > 0) {
        if (userId.isEmpty) {
          billingError = Exception('Missing user id for overtime billing.');
        } else {
          try {
            await _createAutoOvertimeBill(
              bookingId: bookingId,
              userId: userId,
              amount: penalty,
              overtimeHours: overtimeHours,
              issuedAt: dropoffTime,
            );
          } catch (e) {
            billingError = e;
          }
      ...
      final penaltyText = penalty > 0
          ? (billingError == null
              ? ' Auto billing ${_money(penalty)} was created for this overtime penalty.'
              : ' Overtime penalty is ${_money(penalty)}, but the auto billing record could not be created. Please contact admin/staff to issue it manually.')
          : '';
