...
class _PromotionAdminPageState extends State<PromotionAdminPage> {
  ...
  bool _isFutureScheduled(Map<String, dynamic> row, {DateTime? now}) {
    final current = now ?? DateTime.now();
    final start = _parseDate(row['start_at']);
    return start != null && current.isBefore(_startOfDay(start));
  }

  bool _isExpiredRow(Map<String, dynamic> row, {DateTime? now}) {
    final current = now ?? DateTime.now();
    final end = _parseDate(row['end_at']);
    return end != null && current.isAfter(_endOfDay(end));
  }

  bool _isEffectivelyActive(Map<String, dynamic> row, {DateTime? now}) {
    return _isActiveFlag(row['active']) && !_isFutureScheduled(row, now: now) && !_isExpiredRow(row, now: now);
  }

  String _effectiveStatusLabel(Map<String, dynamic> row) {
    if (_isExpiredRow(row)) return 'Expired';
    if (_isFutureScheduled(row)) return 'Scheduled';
    return _isActiveFlag(row['active']) ? 'Active' : 'Inactive';
  }

  Future<void> _syncExpiredRecords({
    ...
    try {
      final rows = await _supa
          .from(table)
          .select('$idColumn,end_at,active')
          .eq('active', true);
      for (final raw in (rows as List)) {
        final row = Map<String, dynamic>.from(raw as Map);
        if (!_isExpiredRow(row)) continue;
        final recordId = (row[idColumn] ?? '').toString().trim();
        if (recordId.isEmpty) continue;
        try {
          await _supa.from(table).update({'active': false}).eq(idColumn, recordId);
        } catch (_) {}
      }
    } catch (_) {}
  }
  ...
  Future<List<Map<String, dynamic>>> _loadPromos() async {
    await _syncExpiredRecords(table: 'promotion', idColumn: 'promo_id');
  ...
  Future<List<Map<String, dynamic>>> _loadAnnouncements() async {
    await _syncExpiredRecords(table: 'announcement', idColumn: 'ann_id');
  ...
  Future<void> _toggleVoucherActive(Map<String, dynamic> p) async {
    final promoId = (p['promo_id'] ?? '').toString().trim();
    if (promoId.isEmpty) return;
    final cur = _isActiveFlag(p['active']);
    final next = !cur;
    if (next && _isExpiredRow(p)) {
      _toast('Expired vouchers cannot be activated again. Update the end date first.', bg: Colors.orange);
      return;
    }
    try {
      await _supa.from('promotion').update({'active': next}).eq('promo_id', promoId);
      await _refresh();
    } catch (e) {
  ...
  Future<void> _toggleAnnouncementActive(Map<String, dynamic> a) async {
    final annId = (a['ann_id'] ?? '').toString().trim();
    if (annId.isEmpty) return;
    final cur = _isActiveFlag(a['active']);
    final next = !cur;
    if (next && _isExpiredRow(a)) {
      _toast('Expired announcements cannot be activated again. Update the end date first.', bg: Colors.orange);
      return;
    }
    try {
      await _supa.from('announcement').update({'active': next}).eq('ann_id', annId);
      await _refresh();
    } catch (e) {
      _toast('Update failed: $e', bg: Colors.red);
    }
              ...
              builder: (context, snap) {
                ...
                return Column(
                  children: rows.map((p) {
                    ...
                    final effectiveStatus = _effectiveStatusLabel(p);
              ...
              builder: (context, snap) {
                ...
                return Column(
                  children: rows.map((a) {
                    ...
                    final effectiveStatus = _effectiveStatusLabel(a);
